...
// Read and rewrite dist/index.html
if (fs.existsSync(path.join(__dirname, 'dist/index.html'))) {
    ...
    fs.writeFileSync(path.join(__dirname, 'dist/index.html'), indexContent, 'utf8');
}


// Read and rewrite dist/what-we-do/index.html
if (fs.existsSync(path.join(__dirname, 'dist/what-we-do/index.html'))) {
    let wwdContent = fs.readFileSync(path.join(__dirname, 'dist/what-we-do/index.html'), 'utf8');
    for (const [regex, replacement] of wwdReplacements) {
        wwdContent = wwdContent.replace(regex, replacement);
    }
    fs.writeFileSync(path.join(__dirname, 'dist/what-we-do/index.html'), wwdContent, 'utf8');
}

console.log("Applied Heritage Ministries United text updates to dist folder.");
